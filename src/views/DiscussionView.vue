...
async function handleViewModeChange(mode: DiscussionViewMode): Promise<void> {
  viewMode.value = mode
  if (mode === 'vehicleKnowledge') return
  await discussionStore.subscribeToSort(mode, authStore.user?.id)
}
...
<template>
  <div class="discussion-shell">
    ...
    <div class="content">
      <DiscussionFilter :model-value="viewMode" @update:model-value="handleViewModeChange" />

      <template v-if="viewMode !== 'vehicleKnowledge'">
        <input
          v-if="searchOpen"
          v-model="searchQuery"
          class="search-input"
      ...
      <VehicleKnowledgeSection v-else />
    </div>
